...
def getProductos(cursor: psycopg2.extensions.cursor = Depends(getCursor)):
    res = ProdManager.getProductos(cursor)
    return res

@router.post("/crear_productos")
def postProductos(producto: ProductoModel, cursor: psycopg2.extensions.cursor = Depends(getCursor)):
    res = ProdManager.addProducto(producto, cursor)
    return {"msg": res}

@router.put("/modificar_producto/{id}")
def modificarProducto(id: int, producto: ProductoUpdateModel, cursor: psycopg2.extensions.cursor = Depends(getCursor)):
